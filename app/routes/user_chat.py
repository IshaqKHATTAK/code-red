...
async def send_message(data: UserSecureChat, backgound_task:BackgroundTasks, session: Session = Depends(database_config.get_async_db)): 
    ...
    bot_answer, thread_id, image_generation_flag =  await user_chat.chat_with_external_bot(data, session, background_tasks=backgound_task)
    return ExternalChatbotResponse(id=thread_id, answer=bot_answer['message'] if not image_generation_flag else '', image_generation_flag=image_generation_flag)
...
async def generate_s3_presigned_url(
    ...
    try:
        ...
        return S3Response(files=presigned_urls)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def all_thread(
    ...
    formatted_results = await user_chat.get_all_org_session(user=current_user, skip=skip, limit=limit, db=session, chatbot_ids=filters_data.chatbot_ids, user_ids=filters_data.user_ids)
