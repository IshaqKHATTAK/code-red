...
from app.schemas.response.organization import OrganizationResponse, ChatbotInfo, OrganizationUsersResponse
...
from app.schemas.response.organization import SuperAdminOrganizations, SuperAdminOrganizationsAll, AllSuperAdminOrganizationsWithout, AllSuperAdminOrganizations
from app.utils.database_helper import insert_document_entry,delete_document_entry,delete_webscrap_entry, insert_webscrap_entry, get_webscrap_entery
...
from app.utils.db_helpers import get_user_by_email, get_user_by_id, get_organization_external_chatbot
from app.schemas.response.chatbot_config import ChatbotPrompt, WebscrapUrl, DocumentInfo, QATemplateData, ChatbotFileUpdateResponse, PromptData, ChatbotConfigResponse, Supervisors,GetGuardrails
...
async def create_bulk_user_service(
    ...
    required_columns = ['Full Name', 'Email', 'Internal Chatbots', 'Password']
...
async def list_all_organizations_service_without_pagination(
    db: AsyncSession,
    current_user: User
):
        ...
        if admin_user:
            response.append(
                SuperAdminOrganizationsAll(
                    ...
                    allowed_dept_chatbots = org.allowed_dept_chatbots,
                    image_generation = org.is_image_generation_allow if org.is_image_generation_allow else False
                    # is_paid = admin_user.is_paid,
                    # current_plan = admin_user.current_plan
                )
            )
    
    return AllSuperAdminOrganizationsWithout(
...
from app.utils.db_helpers import get_user_organization_admin, get_user_by_id
...
async def create_organization_user_service(
    ...
    if not user_data.password:
        user_data.password = "sample_password"
        user_data.confirm_password = "sample_password"
    else:
    ...
    #         raise HTTPException(
    #             status_code=400,
    #             detail="Free users without a paid plan cannot create a users."
    #         )

        # if (len(users)-1) >= envs.FREE_TIER_USERS:
        #     raise HTTPException(
        #         status_code=400,
        #         detail="Please upgrade your plan to create a chatbot."
        #     )
    ...
    for chatid in user_data.chatbot_ids:
            ...
            if chatbot:
                # user_chatbots.append(int(chatbot.id))
    ...
    print("User Chatbots:", user_chatbots, type(user_chatbots))
    ...
    userresponse = UserResponse(
                ...
                user_job = created_user.job_role
            )
    from app.common.security import SecurityManager
    verification_token = SecurityManager.create_verification_token(created_user)
    from app.services import email
    #Send verification email in background
    background_tasks.add_task(
        email.send_set_password_email,
        created_user.email,
        created_user.name,
        verification_token
    )
...
async def super_admin_list_organization_users_service(
    ...
    return response
from app.schemas.response.organization import UpdateOrganization, UpdateUserProfile, AdminiUpdateUserProfile
...
async def fetch_organization_profile(db: AsyncSession, organization_id: int, current_user):
    ...
    if current_user.role == UserRole.ADMIN:
        return UpdateOrganization(
            ...
            allowed_dept_chatbots = organization.allowed_dept_chatbots,
            can_create_external_bot = organization.can_create_external_bot,
            # chat_model_name=organization.chat_model_name,
            is_image_generation_allwed = organization.is_image_generation_allow
        ).dict(exclude_none=True)
    
    print(f'organization {organization.is_image_generation_allow} -- {organization.allowed_dept_chatbots}')
    return UpdateOrganization(
        name=organization.name,
        logo=organization.logo,
        website_url=organization.website_link,
...
async def admin_update_user_profile(
    ...
    if updated_data.avatar_url is not None and updated_data.avatar_url != user.avatar_url:
        ...
        updated = True
    org = await db.execute(select(Organization).filter(Organization.id == current_user.organization_id))
    org = org.scalars().first()
    if updated_data.website_link:
        org.website_link = updated_data.website_link
        await db.commit()
    ...
    return AdminiUpdateUserProfile(
        name=user.name,
...
async def admin_update_user_profile(
    ...
    result = await db.execute(select(User).filter(User.id == admin_id))
    user = result.scalars().first()
    ...
    if updated_data.password:
        hashed_password = SecurityManager().get_password_hash(updated_data.password)
...
async def update_organization_profile_data(
    ...
    if current_user.role == UserRole.ADMIN:
        ...
        if updated_data.is_image_generation_allwed is not None or updated_data.chat_model_name or updated_data.can_create_external_bot:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can't modify your organization chat model configuration."
            )
        if updated_data.allowed_dept_chatbots is not None or updated_data.sso_domain:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You can't modify your organization chat model configuration."
            )
        
        # if organization.can_create_external_bot and updated_data.allowed_dept_chatbots:
        #     raise HTTPException(
        #         status_code=status.HTTP_403_FORBIDDEN,
        #         detail="You can't update the organization bot settings."
        #     )
    ...
    if updated_data.website_url is not None and updated_data.website_url != organization.website_link:
        ...
        updated = True
    if updated_data.can_create_external_bot is not None:
        organization.can_create_external_bot = updated_data.can_create_external_bot
    if updated_data.is_image_generation_allwed is not None:
        organization.is_image_generation_allow = updated_data.is_image_generation_allwed
    if updated_data.chat_model_name is not None:
        organization.chat_model_name = updated_data.chat_model_name
    if updated_data.allowed_dept_chatbots is not None:
        if updated_data.allowed_dept_chatbots != organization.allowed_dept_chatbots:
            if organization.allowed_dept_chatbots and updated_data.allowed_dept_chatbots:
                if updated_data.allowed_dept_chatbots <= organization.allowed_dept_chatbots:
                    from app.services.payment import adjust_tier_degration_change
                    await adjust_tier_degration_change(db, organization_id, updated_data.allowed_dept_chatbots, current_user)
            organization.allowed_dept_chatbots = updated_data.allowed_dept_chatbots
    
    # if not updated: is_image_generation_allwed
    #     raise HTTPException(
    #         status_code=status.HTTP_400_BAD_REQUEST,
    ...
    await db.commit()
    await db.refresh(organization)
    if not organization.can_create_external_bot:
        external_bot = await get_organization_external_chatbot(db, organization_id)
        if external_bot:
            await remove_chatbot_from_organization_service(db, organization_id, external_bot.id, current_user)
    else:
        external_bot = await get_organization_external_chatbot(db, organization_id)
        if not external_bot:
            from app.services.user import create_external_chatbot
            await create_external_chatbot(organization_id, db)
...
async def list_organization_users_service(
    ...
    for user in users:
        ...
        for chatid in user.chatbot_ids:
            ...
            chatbot = chatbots_result.scalar_one_or_none()
            if chatbot:
                if chatbot.chatbot_type == 'Department':
                    user_chatbots.append(ChatbotInfo(
...
async def update_organization_user_status_service(
    ...
    print(f'user job == {update_data["user_job"]}')
    user.job_role = update_data["user_job"]
    await db.commit()
    await db.refresh(user)
    user_chatbots = []
    for chatid in user.chatbot_ids:
    ...
    users_response=OrganizationResponse(
                id=user.id,
                name=user.name,
                email=user.email,
                total_messages=user.total_messages,
...
async def create_organization_chatbot_service(
    ...
    if user_role == UserRole.ADMIN and current_user.role == UserRole.ADMIN:
        if current_user.organization_id != organization_id:
            raise HTTPException(
                status_code=403,
    ...
    # if user_role == UserRole.ADMIN and current_user.current_plan == Plan.tier_1:
    #     raise HTTPException(
    #         status_code=403,
    ...
    query = select(ChatbotConfig).filter(
        ...
        ChatbotConfig.chatbot_type == "Department"
    )
    ...
    org_data = await get_organization(db = db, organization_id = organization_id)
    if len(chatbotnames) >= org_data.allowed_dept_chatbots:
        raise HTTPException(
            status_code=403,
            detail="You reached your chatbots limit."
        )
    ...
    #     )
    # if (len(admin_chatbots)- 1) >= envs.TEIR3_CHATBOTS and current_user.current_plan == Plan.tier_3:
    #     raise HTTPException(
    #         status_code=403,
    ...
    chatbot = ChatbotConfig(
        ...
        llm_prompt=[prompt.dict() for prompt in chatbot_data.llm_prompt]
    )
    db.add(chatbot)
    await db.commit()
    ...
    if chatbot_data.llm_prompt:
        ...
        for prompt_portion in chatbot_data.llm_prompt:
            total_prompt += f"{prompt_portion.title}:{prompt_portion.text}"
    ...
    if website_links:
        for website_link in website_links:
            if website_link.sweep_domain:
                entery_added = await insert_webscrap_entry(chatbot_id = chatbot.id, url = website_link.website_link, sweap_domain = website_link.sweep_domain, content_type = 'url',status = 'Uploaded',session = db)
            else:
                entery_added = await insert_webscrap_entry(chatbot_id = chatbot.id, url = website_link.website_link, sweap_domain = False, content_type = 'url',status = 'Uploaded',session = db)
            message_body = {
    ...
    for supervisor_id in supervisors:
        user_data = await get_user_by_id(db = db, user_id = supervisor_id)
        if user_data:
            if user_data.supervisor_chatbot_ids is None:
                user_data.supervisor_chatbot_ids = []
            if chatbot.id not in user_data.supervisor_chatbot_ids:
                user_data.supervisor_chatbot_ids.append(chatbot.id)
            # if user_data.chatbot_ids is None:
            #     user_data.chatbot_ids = []
            # if chatbot.id not in user_data.chatbot_ids:
            #     user_data.chatbot_ids.append(chatbot.id)
            
            
            flag_modified(user_data, "supervisor_chatbot_ids")
            # flag_modified(user_data, "chatbot_ids")
            db.add(user_data)
        await db.commit() 
...
async def list_organization_chatbots_service(
    ...
    if current_user.role == UserRole.USER:
        if not current_user.chatbot_ids and not current_user.supervisor_chatbot_ids:
            return []
        chatbot_ids = current_user.chatbot_ids or []
        supervisor_chatbot_ids = current_user.supervisor_chatbot_ids or []
        all_chatbot_ids = set(chatbot_ids + supervisor_chatbot_ids)
        query = (
        select(ChatbotConfig)
            .filter(
                ChatbotConfig.organization_id == organization_id,
                ChatbotConfig.id.in_(all_chatbot_ids)  
            )
        )
        ...
        #                 .filter(
        #                     ChatbotConfig.organization_id == organization_id,
        #                     ChatbotConfig.id.in_(current_user.chatbot_ids)  
        #                 )
        #             )
...
async def get_organization_chatbot_service(
    ...
    # try:
    await _role_based_checks_includes_supervisor(current_user, organization_id, chatbot_id)
    # await _role_based_checks(current_user, organization_id)
    query = (
        select(ChatbotConfig)
        .filter(
    ...
    from app.utils.db_helpers import get_bot_supervisors
    supervisors = await get_bot_supervisors(db = db, chatbot_id = chatbot.id)
    supervisors_data = [
            Supervisors(
                name = supe.name,
                supervisor_id = supe.id
            )
    ...
    await db.commit()
    formatted_prompt = []
    if chatbot.llm_prompt:
        for prompt_item in chatbot.llm_prompt:
            formatted_prompt.append(PromptData(
...
async def update_organization_chatbot_details(
    ...
    if current_user.role != UserRole.USER:
        existing_supervisors_query = select(User).filter(User.supervisor_chatbot_ids.contains([chatbot.id]))
        existing_supervisors_result = await db.execute(existing_supervisors_query)
        existing_supervisors = existing_supervisors_result.scalars().all()

        for old_supervisor in existing_supervisors:
            if chatbot.id in old_supervisor.supervisor_chatbot_ids:
                old_supervisor.supervisor_chatbot_ids.remove(chatbot.id)  # Remove chatbot ID
                flag_modified(old_supervisor, "supervisor_chatbot_ids")
                db.add(old_supervisor)
        await db.commit()
        for supervisor_id in supervisors:
            user_data = await get_user_by_id(db = db, user_id = supervisor_id)
            if user_data:
...
#                 raise HTTPException(
#                     status_code=423,
#                     detail="No chatbot available to chat."
#                 )
#             return chatbot[0].llm_model_name
...
async def toggle_organization_image_generation(organization_id: int, db, current_user):
    if current_user.role == UserRole.SUPER_ADMIN:
        organization_detials = await get_organization(db=db,organization_id=organization_id)
    elif current_user.role == UserRole.ADMIN:
        if current_user.organization_id != organization_id:
            raise HTTPException(
                status_code=443,
    ...
    organization_detials.is_image_generation_allow = not organization_detials.is_image_generation_allow
    await db.commit()
    await db.refresh(organization_detials)
    return organization_detials.is_image_generation_allow
...
async def update_organization_chatbot_model_name(
    ...
    try:
        print(f'organization model == {model_name}')
        organization = await get_organization(db=db,organization_id=org_id)
        organization.chat_model_name = model_name
        db.add(organization)
        # all_chatbots_of_org = await list_organization_chatbots_service(db=db, organization_id=org_id,current_user=current_user)
        # for chatbot_data in all_chatbots_of_org:
        #     chatbot_data.llm_model_name = model_name
        #     db.add(chatbot_data)
        ...
        await db.commit()
        await db.refresh(organization)
        print(f'new model added == {organization.chat_model_name}')
        return
    except HTTPException as http_err:
        raise http_err
    except SQLAlchemyError as db_err:
...
async def update_organization_chatbot_traning_text(
    ...
    chatbot.llm_prompt=[prompt.dict() for prompt in training_text]
    chatbot.prompt_status = 'Uploaded'
    db.add(chatbot)
    await db.commit()
    await db.refresh(chatbot)
    #DELETE AND RE-ADD the values.
...
async def get_total_chatbot_filesize(
    ...
    return {"total_files_size":total_size_in_kb,"total_size_in_kb":50000}
    ...
    if website_added:
        ...
        for website_link in website_added:
            if website_link.website_url_id:
                ...
                database_entry = await insert_webscrap_entry(chatbot_id = chatbot.id, url = website_link.website_link, sweap_domain = website_link.sweep_domain, content_type = 'url',status = 'Uploaded',session = db)
                message_body = {
                    "url": website_link.website_link,
                    "org_id": chatbot.organization_id,
                    "chatbot_id": chatbot.id,
                ...
                database_entry = await insert_webscrap_entry(chatbot_id = chatbot.id, url = website_link.website_link, sweap_domain = False, content_type = 'url',status = 'Uploaded',session = db)
                urls.append(
                    WebscrapUrl(
...
async def remove_chatbot_from_organization_service(db, organization_id, chatbot_id, current_user):
    ...
    from app.utils.db_helpers import get_bot_supervisors, get_bot_users
    supervisors = await get_bot_supervisors(db, chatbot_id)
    users = await get_bot_users(db, chatbot_id)

    for supervisor in supervisors:
        if supervisor.supervisor_chatbot_ids and chatbot_id in supervisor.supervisor_chatbot_ids:
            supervisor.supervisor_chatbot_ids.remove(chatbot_id)
        flag_modified(supervisor, "supervisor_chatbot_ids")
    for user in users:
        if user.chatbot_ids and chatbot_id in user.chatbot_ids:
            user.chatbot_ids.remove(chatbot_id)
        flag_modified(user, "chatbot_ids") 
    await db.commit()
...
async def get_bubble_settings_customization(
    ...
    if not chatbot_customize:
        ...
        response =  BubbleCutomize(bubble_bgColor = "#F9F9F9", bubble_icon = "https://codered-file-upload.s3.eu-north-1.amazonaws.com/logo/dindin.png")
        return response
...
async def get_chatbot_settings_customization(
    ...
    if not chatbot_customize:
        from app.schemas.request.chatbot_config import ChatbotCutomize
        response =  ChatbotCutomize(primary_color = "#117ed0", chat_header = "#FFFFFF", sender_bubble_color = "#D3D3D3", receiver_bubble_color = "#efefef", senderTextColor= "#000000", receiverTextColor="#000000" )
        return response
    return chatbot_customize
...
async def _role_based_checks_includes_supervisor(current_user: User, organization_id, bot_id = None):
    if current_user.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN, UserRole.USER]:
        raise HTTPException(status_code=403, detail="Regular user can't update organization related detials.")
    if current_user.role == UserRole.ADMIN and organization_id != current_user.organization_id:
        raise HTTPException(status_code=403, detail="You can only modify details of your organization.")
    if current_user.role == UserRole.USER:
        ...
        #     raise HTTPException(status_code=403, detail="You can only modify details of your organization.")
        print(f'botd id = {bot_id} and all bot ids = {current_user.supervisor_chatbot_ids}')
        if bot_id not in current_user.supervisor_chatbot_ids:
            print(f'not found bot id')
            raise HTTPException(status_code=403, detail="You can only modify details of your organization.")
    return 
...
async def create_organization(db: AsyncSession, org_data: OrganizationCreate) -> Organization:
    ...
    print(f'org_data.can_create_external_bot = {org_data.can_create_external_bot}')
     
    db_org = Organization(is_image_generation_allow = org_data.is_image_generation_allow,chat_model_name = org_data.chat_model_name,allowed_dept_chatbots = org_data.allowed_dept_chatbots, can_create_external_bot = org_data.can_create_external_bot, name=org_data.name, logo = org_data.logo, website_link = org_data.website_link, sso_domain = org_data.sso_domain)
    db.add(db_org)
    await db.commit()
