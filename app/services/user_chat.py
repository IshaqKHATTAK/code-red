...
from app.utils.database_helper import start_scheduler, increment_org_message_count,increment_chatbot_message_count, increment_public_chatbot_per_day_message_count,increment_chatbot_per_day_message_count,increment_admin_chatbot_message_count
# import redis
...
async def chat_with_external_bot(data: UserSecureChat, session, background_tasks: BackgroundTasks):
    try:
        ...
        # )
        background_tasks.add_task(
            increment_org_message_count,
            org_id = bot_config.organization_id,
            session = session
        )
        organization = await get_organization(db=session, org_id=bot_config.organization_id)
        # llm = load_llm(api_key=envs.OPENAI_API_KEY, name='gpt-4o', temperature=bot_config.llm_temperature)
        llm = load_llm_in_json_mode(api_key=envs.OPENAI_API_KEY, name=organization.chat_model_name if organization.chat_model_name else "gpt-4o", temperature=bot_config.llm_temperature)
        ...
        if data.generate_image:
            ...
            if not organization or not organization.is_image_generation_allow:
                raise HTTPException(
                    status_code=423,
                    detail="Your organization don't have access to image generation. Please contact DINBOT support."
                )
            ...
            qa_chain = await construct_kb_chain(db_session=session, memory_status=chatbot.memory_status, LLM_ROLE=bot_config.llm_role, llm = llm, chatbot_id = int(data.bot_id), user_id=1, org_id =organization_id, user_input = data.question) #  21 10 current_user.id  current_user.organization_id
...
async def chat_with_bot(data: UserChat, session, current_user, background_tasks: BackgroundTasks):
    ...
    if current_user.role == UserRole.ADMIN:            
        ...
        if not chatbot:
            raise HTTPException(status_code=404, detail="Chatbot not found")
        if data.bot_id not in current_user.chatbot_ids and data.bot_id not in current_user.supervisor_chatbot_ids:
            raise HTTPException(status_code=404, detail="Chatbot not found")
    ...
    # if current_user.role == UserRole.ADMIN:
    #     if current_user.current_plan == Plan.free and bot_config.chatbot_type == "External" and bot_config.admin_per_days_messages_count >= envs.ADMIN_MESSAGES_WITH_EXTERNAL_PER_DAY_FREMIUM:
    #         raise HTTPException(status_code=404, detail="Limit reached for chattin with bot. Please upgrade your plans.")
    guirdrails = await _get_guardrails(bot_config.id, session)
    if not bot_config:
        raise HTTPException(status_code=404, detail="Chatbot not found")
    
    organization = await get_organization(db=session, org_id=current_user.organization_id)
    ...
    if len(chat_history) % 10 == 0 and len(chat_history) != 0:
    ...
    if not flag:
        ...
        if data.generate_image:
            ...
            qa_chain = await construct_kb_chain(db_session=session, memory_status=bot_config.memory_status, LLM_ROLE=bot_config.llm_role, llm = llm, chatbot_id = data.bot_id, user_id=current_user.id, org_id =current_user.organization_id , enable_image_generation=True, user_input = data.question)
            print(f'history before invoke == {chat_history}')
            response = await qa_chain.ainvoke({"input": question, "chat_history": chat_history})
            print(f'response from invorke == {response}')
            if response.tool_calls:
            ...
            qa_chain = await construct_kb_chain(db_session=session, memory_status=bot_config.memory_status, LLM_ROLE=bot_config.llm_role, llm = llm, chatbot_id = data.bot_id, user_id=current_user.id, org_id = current_user.organization_id, user_input = data.question) #  21 10  
            print(f'history before invoke == {chat_history}')
            response = await qa_chain.ainvoke({"input": question, "chat_history": chat_history})
            print(f'response from else ==  {response}')
            response = {"role": "assistant", "message": f"{response.answer}"}
    ...
    chatbot_messages = [
        {
            ...
            "chatbot_type": chatbot_data[cid]['type']
        }
        for cid in chatbot_ids
    ]
...
async def get_all_org_session(user, skip, limit, db,  chatbot_ids: List[int] = None, user_ids: List[int] = None):
    if user.role not in [UserRole.ADMIN, UserRole.SUPER_ADMIN]:
        raise HTTPException(status_code=404, detail="Not autherized.")
    # if not chatbot_ids:
    ...
    chatbot_query = await db.execute(
        select(ChatbotConfig.id, ChatbotConfig.chatbot_name, ChatbotConfig.chatbot_type)
        .where(ChatbotConfig.organization_id == user.organization_id)
        .where(ChatbotConfig.chatbot_type == 'Department')
    )
...
async def get_message_feedback(
    ...
    if current_user.role not in [UserRole.ADMIN or UserRole.SUPER_ADMIN]:
        raise HTTPException(status_code=404, detail="Unautherize.")
